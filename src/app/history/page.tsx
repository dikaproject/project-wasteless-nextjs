...
const canCompleteOrder = (transaction: Transaction) => {
...
export default function History() {
  ...
  const handleCompleteOrder = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/customer/transactions/${id}/complete`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        className="text-green-600 hover:text-green-700 text-sm font-medium border border-green-600 px-4 py-2 rounded-lg hover:bg-green-50"
                      >
                        View Details
                      </button>
                      {canCompleteOrder(transaction) && (
                        <button
