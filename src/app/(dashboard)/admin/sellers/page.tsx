...
export default function SellerManagement() {
  ...
  const fetchPendingProducts = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/admin/seller-products`,
        {
          headers: {
  ...
  const handleApprove = async (productId: number) => {
    try {
      setApprovingId(productId); // Set loading state
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/admin/seller-products/${productId}/approve`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleReject = async (productId: number) => {
    ...
    try {
      setRejectingId(productId); // Set loading state
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/admin/seller-products/${productId}/reject`,
        {
          method: "PUT",
          headers: {
              ...
              <div className="mt-6 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4 text-gray-600">
                  ...
                  <div>
                    ...
                    <p>
                      {format(new Date(selectedProduct.expired), "dd MMM yyyy")}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex justify-end space-x-4">
                  ...
                  <button
                    ...
                    disabled={rejectingId === selectedProduct.id}
